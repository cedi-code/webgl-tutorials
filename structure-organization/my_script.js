...
const elementSlider = document.getElementById('elements-slider');
...
const cube = primitives.createCube(1);
const sphere = primitives.createSphere(0.7);
const cone = primitives.createTruncatedCone(0.8, 0.1, 1);
console.log("cube:", cube);
console.log("sphere:", sphere);
console.log("cone:", cone);
// create the buffers
const bufferInfoCube = myWebglUtils.createBufferInfoFromArrays(gl, cube);
const bufferInfoSphere = myWebglUtils.createBufferInfoFromArrays(gl, sphere);
const bufferInfoCone = myWebglUtils.createBufferInfoFromArrays(gl, cone);
const bufferTypes = [bufferInfoCube, bufferInfoSphere, bufferInfoCone];
const objectsToDraw = [];
// create random objects
let projectionMatrix = myMath.perspective(myMath.toRad(30), (gl.canvas.width / gl.canvas.height), 1, 1000);
let viewMatrix = myMath.inverse(myMath.lookAt([0, 10, 15], [0, 0, 0], [0, 1, 0]));
