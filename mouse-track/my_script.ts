...
const lightMoveWindow : number = 10;
let stillTime : number = 0;
const mouseRestTime : number = 0.0;
const lookAtTime : number = 1.0;
let lookPrev : Vector3 = [0,0,-3];
let lookCurr : Vector3 = [0,0,-3];
let lookGoal : Vector3 = [0,0,-3];
...
{
    let x : number = event.clientX / gl.canvas.width;
    let y : number = event.clientY / gl.canvas.height;
