...
function createStaticBuffer(gl : WebGLRenderingContext, type : GLenum, data : number[]) : WebGLBuffer {
...
function setRectangle(
    ...
    const positionBuffer1 = createStaticBuffer(gl, gl.ARRAY_BUFFER, positions2) as WebGLBuffer;
...
const vertexShaderSourceCode : string = await loadShader('shaders/vertex.glsl');
const fragmentShaderSourceCode : string = await loadShader('shaders/fragment.glsl');
...
const resolutionUniformLocation = gl.getUniformLocation(program,"u_resolution") as number;

const colorUniformLocation = gl.getUniformLocation(program, "u_color") as number;
...
const positionBuffer = createStaticBuffer(gl, gl.ARRAY_BUFFER, positions) as WebGLBuffer;
...
const count : GLsizei = 6;          // number of times we execute vertex shader
